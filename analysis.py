...
def run_analysis(df, true_totals):
    ...
    df = df.with_columns(
        pl.col("skills_found").replace({
            "bachelor": "bachelor's degree", "s degree": "bachelor's degree",
            "bachelor s": "bachelor's degree", "bachelor s degree": "bachelor's degree",
            "master": "master's degree", "master s": "master's degree",
            "master s degree": "master's degree", "apis": "api", "c + +": "c++", 
            "c +": "c++", "react. js": "react", "reactjs": "react",
            "node. js": "node.js", ". net": ".net", "robotic": "robotics",
            "machine vision": "computer vision", "verbal communication": "communication",
    ...
    # 3. Join with Golden Denominator
    df_norm = patterns.join(
        true_totals, 
        left_on=[PROFESSION_COL, "jobOpening_workExperienceYears"],
        right_on=["jobOpening_professionFinal", "jobOpening_workExperienceYears"],
    ...
    # DYNAMIC RETURN: Create results for ALL professions found in the dataset
    all_profs = df_norm[PROFESSION_COL].unique().to_list()
    # return {prof: get_ev(prof) for prof in all_profs if prof is not None}
    results_dict = {prof: get_ev(prof) for prof in all_profs if prof is not None}

    return results_dict, df # <--- MUST RETURN df HERE